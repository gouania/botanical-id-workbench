...
import io
import urllib.parse# Configure page with botanical theme
st.set_page_config(
...
""", unsafe_allow_html=True)# --- END: Custom CSS ---

# Suppress warnings
warnings.filterwarnings('ignore')
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)# Initialize session state
...
if 'eflora_data' not in st.session_state:
    st.session_state.eflora_data = None# Caching decorator
def file_cache(cache_dir="cache"):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            cache_file = os.path.join(cache_dir, f"{cache_key}.pkl")        if os.path.exists(cache_file):
        ...
        return result
    return wrapper
return decorator# Core functions from your notebook
@st.cache_data
def download_and_load_eflora():
    ...
    try:
        ...
        missing_files = [f for f in required_files if not os.path.exists(f)]    if missing_files:
    ...
    return Nonedef format_species_name(name):
    if not name: return None
    parts = name.split()
    return f"{parts[0]} {parts[1]}" if len(parts) >= 2 else name@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=1, max=10))
def safe_gbif_backbone(name, kingdom='Plantae'):
    return gbif_species.name_backbone(name=name, kingdom=kingdom, verbose=False)# iNaturalist license code mapping
INAT_LICENSE_MAP = {
    1: 'CC BY',
    2: 'CC BY-SA',
    3: 'CC BY-ND',
...
def get_species_images(species_name, limit=5):
    ...
    return [], Nonedef extract_month_from_date(date_str):
    ...
    try:
        ...
        return Nonedef calculate_phenology(records):
    ...
    return month_counts@file_cache(cache_dir="gbif_cache")
def get_species_list_from_gbif(latitude, longitude, radius_km, taxon_name, record_limit=50000):
    ...
    try:
        ...
        if 'usageKey' not in taxon_info or taxon_info.get('matchType') == 'NONE':
            ...
            return [], []    search_taxon_key = taxon_info['usageKey']
    ...
    return [], []def get_local_eflora_description(scientific_name, eflora_data):
    """Get description from local e-Flora data with fuzzy matching."""
    if eflora_data is None:
        return False, "e-Flora data not available"clean_name = format_species_name(scientific_name)
...
try:
    ...
    return False, f"Error retrieving data for {clean_name}"def create_phenology_chart(phenology_data, species_name):
    """Create a chart using Plotly with botanical color scheme."""
    months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun',
              'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
    counts = [phenology_data.get(i, 0) for i in range(1, 13)]fig = go.Figure(data=go.Bar(
    x=months, 
    y=counts,
    marker=dict(
        color='#4a7c24',
...
return figdef create_species_map(records, species_list, center_lat, center_lon):
...
return m# --- START: Creator Acknowledgment Function ---
def add_footer():
    ...
    """, unsafe_allow_html=True)# --- END: Creator Acknowledgment Function ---
# Main Streamlit App
def main():
    st.title(" Botanical Identification Workbench")
    st.markdown("*Advanced species identification using GBIF data and local flora databases*")# Sidebar for parameters
with st.sidebar:
...
if st.session_state.species_data is None:
    ...
    tab1, tab2, tab3, tab4 = st.tabs([" Species List", " Map View", " Analysis", " Export"])
...
add_footer()if __name__ == "__main__":
    main()
